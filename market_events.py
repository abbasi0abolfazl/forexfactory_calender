...
def get_trading_sessions(event_time):
    ...
    if (
        "London" in sessions
        and "New York" in sessions
        and golden_start <= event_time_obj < golden_end
    ):
...
def get_system_timezone():
    ...
    try:
        ...
        print(
            f"Could not determine system timezone: {e}. Falling back to UTC.")
        return "UTC"
...
def fetch_and_parse_xml(
    ...
    for event in root.findall("event"):
        event_data = {
            ...
            if event.find("forecast") is not None
            else None,
            "previous": event.find("previous").text
            if event.find("previous") is not None
            else None,
        ...
        if user_timezone:
            event_data["time"] = convert_time_to_timezone(
                event_data["time"], to_tz=user_timezone
            )

        all_events.append(event_data)

        if (
            (not currency_filter or event_data["currency"] == currency_filter)
            and (
                not title_filter or title_filter.lower(
                ) in event_data["title"].lower()
            )
            and (not impact_filter or event_data["impact"] == impact_filter)
        ):
...
@click.option(
    "--currency", type=str, help="Filter events by currency (e.g., USD, EUR)."
)
@click.option("--title", type=str, help="Filter events by title (case-insensitive).")
@click.option(
    "--impact", type=str, help="Filter events by impact (e.g., High, Medium, Low)."
)
@click.option(
    "--save-format",
    type=click.Choice(["json", "txt"]),
    default="json",
    help="Save format: json or txt.",
)
@click.option(
    "--output-file",
    type=str,
...
@click.option(
    ...
    type=str,
    default=None,
    help="Convert event times to the specified timezone (e.g., 'America/New_York').",
)
@click.option(
    "--system-timezone",
    is_flag=True,
    help="Use the system's local timezone for event times.",
)
@click.option(
    "--config",
    type=click.Path(exists=True),
    default=None,
    help="Path to a JSON config file to load options from.",
)
def main(
    schedule,
    currency,
    ...
    if schedule:
        ...
        fetch_and_parse_xml(output_file, currency, title,
                            impact, save_format, timezone)
